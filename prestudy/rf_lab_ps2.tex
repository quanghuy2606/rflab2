...
\begin{equation}
\eta_\mathrm{SLmax} = \frac{(1 - |\rho_\mathrm{S}|^2)(1 + |\rho_\mathrm{L}|)^2}
...
\begin{equation}
\eta_\mathrm{SLmin} = \frac{(1 - |\rho_\mathrm{S}|^2)(1 + |\rho_\mathrm{L}|)^2}
{(1 + (|\rho_\mathrm{S}||\rho_\mathrm{L}|)^2},
\end{equation}

\noindent
where $\rho_\mathrm{S}$ and $\rho_\mathrm{S}$ are the source and load reflection 
coefficients. The equations are special forms of an equation, and are used in 
the case when the phase relation of the set-up is not known. The reflection 
coefficients can be calculated from voltage standing wave ratio 
$VSWR$ by 

\begin{equation}
|\rho| = \frac{1 - VSWR}{1 + VSWR}.
\end{equation} 
\noindent
Using the equations, the maximum efficiency of power is $\eta_\mathrm{SLmax} 
= -0.14$ dB, and the minimum efficiency of power transfer is 
$\eta_\mathrm{SLmin} = -1.10$ dB. Therefore, the range of the attenuation due 
to mismatch is -1.1 ... -0.14 dB.
...
a set-up with a power isolator in between the source and the load. The equation 
is
...
\begin{table}[!h]
	...
	\hline
	Bias tee				& BT					& 0.7~dB					& 0.7~dB \\
	Diplexer				& DPX					& 0.4~dB					& 0.4~dB \\
	Low-noise amplifier		& LNA					& ???						& 2.8~dB \\
	4-way power divider		& DIV					& 6.0~dB					& 6.0~dB
	\end{tabular}
	\end{center}
	\vspace*{-12pt}
\end{table}
...
\begin{equation}
F_\mathrm{T} = \frac{SNR_\mathrm{in}}{SNR_\mathrm{out}} 
...
(Comment about the matching.)



\begin{comment}
\begin{enumerate}[label=\alph*), itemsep=0pt]
\item The total noise figure $F_{TOT}$ is

	\begin{equation}
	F_{TOT} = L_{BT} + L_D + F,
	\end{equation}
	
	where $L_{BT}$ and $L_D$ are bias tee and diplexer attenuations, and $F$ is 
	the LNA's noise figure. Therefore, the total noise power $P_N$ from the cold load 
	reaching the spectrum analyser is 
	
	\begin{equation}
	P_N = -159 dBm + F_{TOT} + G ,
	\end{equation}
	
	where $-159$~dBm is the 30 Hz band noise power at room temperature (290 K), and G 
	is the LNA's gain. The spectrum analyser noise level is $P_\mathrm{SA} < -125$~dBm, 
	and so the LNA gain should be at least
	
	\begin{equation}
	G = -125 dBm + 159 dBm - F_{TOT} = 30.1 dB
	\end{equation}
...
\begin{table}[!h]
	\begin{center}
	\caption{A simplified list of some of the key requirements in evolving communication systems.}
	\label{t:234g}
	\renewcommand*{\arraystretch}{1.2}
	\begin{tabular}{lccc}
	\textbf{Parameter}				& \textbf{2G (GSM)}		& \textbf{3G (UMTS)} 	& \textbf{4G (LTE-A)} \\
	\hline
	System frequency [MHz]			& A						& B						& C	 \\
	Number of system bands			& A						& B						& C	 \\
	System bandwidth [MHz]			& A						& B						& C	 \\
	Channel bandwidth [MHz]			& A						& B						& C	 \\
	Multiple-access method			& A						& B						& C	 \\
	Duplexing mode					& A						& B						& C	 \\
	Modulation schemes				& A						& B						& C	 \\
