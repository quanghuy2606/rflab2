...
Gavg = mean(abs(s(find(f >= gsm(1), 1, 'first') : find(f <= gsm(2), 2, 'last')))).^2;
...
text(	mean(gsm(3:4)/1e6), ...
		20*log10(abs(y(2))) - 10, ...
		sprintf('$$G_\\mathrm{max} = %.1f$$ dB ($$f = %.1f$$ MHz)', ...
...
text(	mean(gsm(3:4)/1e6), ...
		20*log10(abs(y(2))) - 25, ...
		sprintf('$$G_\\mathrm{RX,\\;avg} = %.1f$$ dB', 10*log10(Gavg)), ...
		'HorizontalAlignment',  'center', ... 
		'Interpreter',          'latex', ...
		'BackgroundColor',      'w', ...
		'color',				'k');
...
PPP2 = 1.0014 * PP1 + 24.6; %1.005*PP1 + 24.8;
...
ylim([-20 25]);
xlim([-45 0]);
title(sprintf('$$P_\\mathrm{1\\;dB,\\;in} = %+.1f $$ dBm, $$P_\\mathrm{1\\;dB,\\;out} = %+.1f $$ dBm, $$|S_{21}|_\\mathrm{avg} = %.1f $$ dB', PP1(idx), PP2(idx), 0.1 + mean(GG(1:idx))), 'interpreter', 'latex');
ylabel('$$P_\mathrm{out}$$ [dB]', 'interpreter', 'latex');
xlabel('$$P_\mathrm{in}$$ [dBm]', 'interpreter', 'latex');
grid on;
hold off;
